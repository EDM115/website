...
if (url.includes('sign-in/')) {
  ...
  function checkUser () {
    ...
    if (localStorage.getItem('keep') == 'true') {
      ...
      const connect = atob(logvalue).split('|')
    ...
    const logstored = localStorage.getItem('logvalue')
...
if (url.includes('sign-up/')) {
  ...
  function setUser () {
    if (
      url.includes('sign-up/?mail-log=') ||
      url.includes('sign-up/?action=edit')
    ) {
      const url2 = url.split('?')
      const redirect = url.replace(url2[1], '').replace('?', '')
      window.history.replaceState({}, document.title, redirect)
    ...
    if (localStorage.getItem('logvalue') == logvalue2) {
      window.alert(
        'Ce compte existe déjà. Vous voulez sûrement modifier vos informations'
      )
      const redirect = url.replace('sign-up/', 'edit/')
      window.location.href = redirect
    } else {
...
if (url.includes('edit/')) {
  if (url.includes('edit/?nom=')) {
    const url2 = url.split('?')
    const redirect = url.replace(url2[1], 'action=edit')
    window.history.replaceState({}, document.title, redirect)
    ...
    if (localStorage.getItem('logvalue') == null) {
      ...
      const redirect = url.replace('edit/', 'sign-up/')
      window.location.href = redirect
    }
    ...
    const mail = connect[0]
    const pwd = connect[1]
    const nomedit = document.getElementById('nom-edit')
    const prenomedit = document.getElementById('prenom-edit')
    const bdateedit = document.getElementById('bdate-edit')
    const sexeedit = document.getElementById('sexe-edit')
    const tailleedit = document.getElementById('taille-edit')
    const poidsedit = document.getElementById('poids-edit')
    const mailedit = document.getElementById('mail-edit')
    const pwdedit = document.getElementById('pwd-edit')
    nomedit.value = nom
